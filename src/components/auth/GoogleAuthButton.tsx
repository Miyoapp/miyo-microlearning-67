...
export const GoogleAuthButton: React.FC<GoogleAuthButtonProps> = ({ text }) => {
  ...
  const handleGoogleAuth = async () => {
    ...
    try {
      ...
      // Note: User will be redirected, so no success toast needed here
    } catch (error) {
  ...
  const buttonText = text === 'login' ? 'Continuar con Google' : 'Registrarse con Google';
  ...
  return (
    ...
    >
      ...
      {loading ? 'Autenticando...' : buttonText}
    </Button>
  );
};
